...
# Use raw strings (prefix with 'r') to avoid invalid escape sequence warnings
p = re.compile(r'COV:(\d*\.?\d+)')
snp_p = re.compile(r'BASERANGE:(\d+)-(\d+)')
hapq_p = re.compile(r'HAPQ:(\d+)')
index_p = re.compile(r'HAP(\d+)')
